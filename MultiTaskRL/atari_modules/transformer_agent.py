...
from atari_modules.models import TaskAwareCritic, RewardEncoder, TransformerCritic  # , RewardEncoderTranslator
...
class TransformerAgent:
    def __init__(self, args, env, env_params):
        ...
        self.max_examples = 200 # this is due to memory concerns
        ...
        with open(os.path.join(dir, "transformer.txt"), "wt") as file:
            file.write("")
    ...
    def act(self, obs, goal, target_network=False, print_q=False):
        ...
        example_states = self.states[goal_indicies, permutation, :]
        example_rewards = self.rewards[goal_indicies, permutation, :]
        if len(example_rewards.shape) == 2:
    ...
    def _value_loss(self, goals, reward_function):
        with torch.no_grad():
            ...
            #  for every state, generate a random set of data to use
            permutation =  torch.cat([torch.randperm(self.states.shape[1], device=self.device)[:self.max_examples].view(1, -1) for s in range(obs_tensor.shape[0])])

            example_states = torch.gather(self.states, 1, permutation.unsqueeze(-1).expand(-1, -1, self.states.shape[-1]))
            example_rewards = torch.gather(self.rewards, 1, permutation.unsqueeze(-1).expand(-1, -1, self.rewards.shape[-1]))
            ...
            # the q loss
        real_q_value = self.critic_network(obs_tensor, example_states, example_rewards)
    ...
    def _update_network(self):
        ...
        for index in range(num_goals_each_time):
            ...
            reward_function = get_reward_function(goals3, self.num_actions)
            loss = self._value_loss(goals3, reward_function)
            loss.backward()
            with torch.no_grad():
