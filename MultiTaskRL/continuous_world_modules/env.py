...
class ContinuousWorld(object):
    ...
    def sample_goal(self):
        type = "random"
        if type == "constant":
            return Point(0.75, 0.25)
        elif type == '2':
            points = [Point(0.75, 0.25), Point(0.05, 0.25)]
            sampled_indices = np.random.choice(len(points), 1)[0]
    ...
    def step(self, id_action) -> Tuple[Tuple[float, float], Optional[float], bool, Dict[str, Any]]:
        ...
        # reward = 0.0 if self._current_position.is_close_to(self._goal, diff=self.threshold_distance) else -0.1
        reward = 0
        done = False # True if reward == 1.0 else False
